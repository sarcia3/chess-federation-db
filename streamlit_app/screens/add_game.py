...
def render():
    ...
    # selectbox filters by the *label* as you type, so searching "magnus"
    # matches "Magnus Carlsen" — and we still get player_id back.
    white = col1.selectbox("White", players, format_func=lambda p: p["name"], key="white")
    black = col2.selectbox("Black", players, format_func=lambda p: p["name"], key="black")
